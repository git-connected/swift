...
:: TODO(compnerd) remove this workaround
git -C "%SourceRoot%\swift-installer-scripts" cherry-pick --abort
git -C "%SourceRoot%\swift-installer-scripts" reset --hard

call "%SourceRoot%\swift\utils\update-checkout.cmd" %args% --clone --skip-tags --skip-history --github-comment "%ghprbCommentBody%"

:: TODO(compnerd) remove this workaround
git -C "%SourceRoot%\swift-installer-scripts" fetch origin pull/18/merge
